  ...
  TForm1 = class(TForm)
    ...
    procedure yearbox1Change(Sender: TObject);
	 ...
	 procedure updateStatus;
         Function workingDays (dtFirst, dtLast: TdateTime): integer;
  private
...
begin
	...
	str(trunc(t3), nbrd);
	form1.StatusBar1.Panels[0].text := 'Number of days: ' + nbrd;
        nbrd := intToStr(workingDays(t1, t2));
        form1.StatusBar1.Panels[1].Text := 'Number of work days: ' + nbrd;
end;
...
begin
	...
	month2.caption := months[calendar2.month] + ' '+y;
        ...
        yearbox2.Text := IntToStr(Year);
        if month = 1 then
                yearbox1.Text := IntToStr(Year -1)
        else
...
Function TForm1.workingDays (dtFirst, dtLast: TdateTime): integer;
(*Returns number of days (pos or neg) from dtFirst to dtLast, inclusive,
minus all Saturdays and Sundays*)
Var
    n: integer;
Begin
    result := 0;
//move both times to 0:0
    dtFirst := int(dtFirst);
    dtLast := int(dtLast);
//move dtFirst to next Sunday
    n := 7 - dayOfWeek(dtFirst);
    if (n<5) then inc(result, n)
    else inc(result, 5);
    dtFirst := dtFirst + n + 1;
//move dtLast to previous or same Sunday
    n := dayOfWeek(dtLast) - 1;
    if (n<5) then inc(result, n)
    else inc(result, 5);
    dtLast := dtLast - n;
//add five days a week to result
    n := trunc(dtLast - dtFirst) div 7;
    inc(result, 5 * n);
End;
